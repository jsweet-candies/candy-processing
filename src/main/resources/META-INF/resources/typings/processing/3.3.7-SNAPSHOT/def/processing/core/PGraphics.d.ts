...
declare class PGraphics extends PImage implements PConstants {
    ...
    static __static_initializer_0() {
        PGraphics.sinLUT = (s => { let a=[]; while(s-->0) a.push(0); return a; })(PGraphics.SINCOS_LENGTH);
        PGraphics.cosLUT = (s => { let a=[]; while(s-->0) a.push(0); return a; })(PGraphics.SINCOS_LENGTH);
        for(var i : number; i < PGraphics.SINCOS_LENGTH; i++) {
            PGraphics.sinLUT[i] = (<any>Math).fround(Math.sin((<any>Math).fround((<any>Math).fround(i * PConstants.DEG_TO_RAD) * PGraphics.SINCOS_PRECISION)));
            PGraphics.cosLUT[i] = (<any>Math).fround(Math.cos((<any>Math).fround((<any>Math).fround(i * PConstants.DEG_TO_RAD) * PGraphics.SINCOS_PRECISION)));
        };
    }
...
declare namespace PGraphics {
    ...
    export class AsyncImageSaver {
        ...
        targetPool : java.util.concurrent.BlockingQueue<PImage>;

        saveExecutor : java.util.concurrent.ExecutorService;
        ...
        public saveTargetAsync(renderer : PGraphics, target : PImage, file : java.io.File);

        public awaitAsyncSaveCompletion(file : java.io.File);
    }
}
